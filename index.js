...
function PraesentationPage() {
  const totalDuration = praesentationOutline.abschnitte.reduce((s, a) => s + a.schaetzteDauer, 0);
  mount(`
    <div class="min-h-screen">
      <div class="max-w-6xl mx-auto px-4 py-8">
        <div class="text-center mb-12" data-reveal>
          <h1 class="text-4xl font-bold text-gray-900 mb-4">Präsentations-Outline</h1>
          <p class="text-xl text-gray-600 max-w-3xl mx-auto">Klare Struktur für 15 Minuten Theorie: einfach zu präsentieren und zu folgen.</p>
          <div class="flex justify-center items-center mt-4 space-x-4">
            <div class="flex items-center text-gray-600">
              <i data-lucide="clock" class="w-5 h-5 mr-2"></i>
              <span>Gesamtdauer: ${totalDuration} Minuten</span>
            </div>
          </div>
        </div>

        <div class="bg-white rounded-lg shadow-md p-6 mb-8" data-reveal>
          <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <div class="flex items-center space-x-4">
              <div id="timerDisplay" class="text-3xl font-mono font-bold text-blue-700">00:00</div>
              <div class="flex space-x-2">
                <button id="toggleTimer" class="flex items-center px-4 py-2 rounded-md font-medium bg-blue-700 text-white hover:bg-blue-800"><i data-lucide="play" class="w-4 h-4 mr-2"></i>Start</button>
                <button id="resetTimer" class="flex items-center px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-800"><i data-lucide="rotate-ccw" class="w-4 h-4 mr-2"></i>Reset</button>
              </div>
            </div>
            <div class="flex space-x-4">
              <button id="printBtn" class="flex items-center px-4 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-800"><i data-lucide="printer" class="w-4 h-4 mr-2"></i>Drucken</button>
              <button id="pdfBtn" class="flex items-center px-4 py-2 bg-black text-white rounded-md hover:bg-gray-900"><i data-lucide="download" class="w-4 h-4 mr-2"></i>PDF Download</button>
            </div>
          </div>
          <div class="mt-6">
            <div class="flex justify-between text-sm text-gray-600 mb-2">
              <span id="currentAbschnittLabel">Aktueller Abschnitt: –</span>
              <span id="progressPct">0% abgeschlossen</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
              <div id="progressBar" class="bg-blue-700 h-2 rounded-full transition-all duration-300" style="width:0%"></div>
            </div>
          </div>
        </div>

        <div id="outline" class="space-y-6">
          ${praesentationOutline.abschnitte
            .map(
              (a, i) => `
              <div class="bg-white rounded-lg shadow-md p-8" data-reveal>
                <div class="flex items-start justify-between mb-6">
                  <div class="flex items-start">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg mr-4 ${i === 0 ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}">${i + 1}</div>
                    <div>
                      <h2 class="text-2xl font-bold text-gray-900 mb-2">${a.titel}</h2>
                      <div class="flex items-center text-gray-600"><i data-lucide="clock" class="w-4 h-4 mr-2"></i><span>Geschätzte Dauer: ${a.schaetzteDauer} Minuten</span></div>
                    </div>
                  </div>
                  <div class="text-right text-sm text-gray-500">Startzeit: ${formatTime(Math.floor(i * (totalDuration * 60) / praesentationOutline.abschnitte.length))}</div>
                </div>
                <ul class="space-y-3 ml-14">
                  ${a.unterpunkte
                    .map((p) => `<li class="flex items-start"><span class="w-2 h-2 bg-blue-600 rounded-full mt-2 mr-3"></span><span class="text-gray-700 text-lg">${p}</span></li>`)
                    .join('')}
                </ul>
              </div>`
            )
  ...
  let seconds = 0;
  let timerId = null;
  const outlineEls = qsa('#outline > div');

  function tick() {
    seconds += 1;
    qs('#timerDisplay').textContent = formatTime(seconds);
    const pct = Math.min((seconds / (totalDuration * 60)) * 100, 100);
    qs('#progressBar').style.width = `${pct}%`;
    qs('#progressPct').textContent = `${Math.round(pct)}% abgeschlossen`;
    const perSection = Math.floor((totalDuration * 60) / outlineEls.length);
    const currentIndex = Math.min(Math.floor(seconds / perSection), outlineEls.length - 1);
    qs('#currentAbschnittLabel').textContent = `Aktueller Abschnitt: ${praesentationOutline.abschnitte[currentIndex].titel}`;
    outlineEls.forEach((el, i) => {
      el.classList.toggle('ring-2', i === currentIndex);
      el.classList.toggle('ring-blue-500', i === currentIndex);
      el.classList.toggle('bg-blue-50', i === currentIndex);
    });
  }

  qs('#toggleTimer').addEventListener('click', () => {
    isRunning = !isRunning;
    if (isRunning) {
      qs('#toggleTimer').innerHTML = '<i data-lucide="pause" class="w-4 h-4 mr-2"></i>Pause';
      timerId = setInterval(tick, 1000);
    } else {
      qs('#toggleTimer').innerHTML = '<i data-lucide="play" class="w-4 h-4 mr-2"></i>Start';
      clearInterval(timerId);
    }
    lucide.createIcons();
  });

  qs('#resetTimer').addEventListener('click', () => {
    clearInterval(timerId);
    isRunning = false;
    seconds = 0;
    ...
    qs('#progressPct').textContent = '0% abgeschlossen';
    qs('#currentAbschnittLabel').textContent = 'Aktueller Abschnitt: –';
  ...
  qs('#pdfBtn').addEventListener('click', () => alert('PDF-Download wird vorbereitet...'));
}
...
function router() {
  ...
  if (hash.startsWith('#/praesentation')) return PraesentationPage();
  return HomePage();
}
